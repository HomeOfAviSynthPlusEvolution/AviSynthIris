...
void errors(AvsApi& api, AVS_ScriptEnvironment* env) {
  ...
  auto result = api.avs_invoke(
      env, "Eval", make_avs_string("IrisPoC(BlankClip(pixel_type=\"YV12\"), \"x.Missing\", \"scalar\", false)"),
      nullptr);
  if (avs_is_error(result)) {
    api.avs_release_value(result);
    throw std::runtime_error("missing property should fail at frame time");
  }
  AvsClip clip(api, api.avs_take_clip(result, env));
  api.avs_release_value(result);
  AvsFrame frame(api, api.avs_get_frame(clip.clip, 0));
  auto* error = api.avs_clip_get_error(clip.clip);
  if (frame.frame || !error || std::string(error).find("Missing") == std::string::npos)
    throw std::runtime_error("missing property error was not propagated through AVS");
}
} // namespace
